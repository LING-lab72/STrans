...
本节补充系统设计报告中的关键图示，包括分层架构图、功能架构图、功能模块图和数据库 ER 图。图示用于支撑系统结构说明、模块分工、数据流转和数据库设计。

### 12.1 分层架构图

系统采用端、边、云协同的分层架构。端侧负责视频采集，边缘侧负责拉流、调度、状态管理和前端展示，算法侧负责目标检测、跟踪、车牌识别和交通分析。开发阶段算法服务可与后端部署在同一台电脑，演示或扩展阶段可迁移到 GPU 主机。

```mermaid
flowchart TB
    subgraph Device["端侧采集层"]
        Phone["手机 IP Webcam / Iriun"]
        ESP32["ESP32-CAM"]
        LocalVideo["本地沙盘视频段"]
    end
    ...
    subgraph Edge["边缘服务层"]
        ...
        VideoService["视频流管理\nOpenCV 拉流 / MJPEG 转发"]
        StateService["系统状态与配置管理"]
        Storage["SQLite / JSONL 持久化"]
    end

    subgraph Algorithm["算法推理层"]
        YOLO["YOLOv11 车辆检测"]
        ByteTrack["ByteTrack 车辆跟踪"]
        OCR["HyperLPR3 / 电子 ID 识别"]
        S2M["S2M 障碍物分割增强"]
        RuleEngine["拥堵 / 禁停 / 闸机规则"]
    end

    subgraph Web["Web 展示层"]
        Dashboard["实时监控 Dashboard"]
        Devices["设备管理"]
        Models["模型与参数管理"]
        Events["事件告警与历史统计"]
    end

    Phone -->|HTTP MJPEG / RTSP| VideoService
    ESP32 -->|HTTP Stream| VideoService
    LocalVideo -->|MP4 文件| VideoService
    ...
    S2M --> RuleEngine
    RuleEngine --> FastAPI
    FastAPI --> StateService
    FastAPI --> Storage
    FastAPI -->|REST / MJPEG| Dashboard
    FastAPI --> Devices
    FastAPI --> Models
    FastAPI --> Events
```
...
功能架构围绕“视频接入、智能分析、规则决策、数据管理、可视化展示”五类能力展开。当前版本已完成视频接入、浅色沙盘大屏 Dashboard、本地 YOLO 测试、设备/模型/系统状态/白名单接口雏形，后续重点完善 ByteTrack、ROI、禁停、S2M 障碍物增强和历史落盘。

```mermaid
flowchart LR
    System["智慧交通视觉感知系统"]

    System --> Video["视频接入与管理"]
    System --> Analysis["视觉分析"]
    System --> Decision["交通规则与决策"]
    System --> Data["数据管理"]
    System --> View["前端可视化"]
    System --> Ops["系统管理"]

    Video --> Video1["手机视频流接入"]
    Video --> Video2["ESP32-CAM 接入"]
    Video --> Video3["本地视频段测试"]
    Video --> Video4["视频状态监测"]

    Analysis --> A1["YOLOv11 车辆检测"]
    Analysis --> A2["ByteTrack 车辆跟踪"]
    Analysis --> A3["车牌/电子 ID 识别"]
    Analysis --> A4["障碍物识别 / S2M 增强"]
...
后端按照接口层、服务层、算法适配层和数据层划分；前端按照页面、组件和 API 访问划分。算法模块由同伴负责训练和算法优化，本系统通过标准接口接入其输出结果。

```mermaid
flowchart TB
    subgraph Frontend["前端模块"]
        FPage["页面层\nDashboard / 设备 / 模型 / 历史"]
        FComp["组件层\nVideoPanel / StatsPanel / EventList / ModelPanel"]
        FApi["API 调用层\nfetch REST / MJPEG 显示"]
    end

    subgraph Backend["后端模块"]
        Api["接口层\n/api/video / analysis / devices / models / whitelist"]
        Video["视频服务\nVideoStreamService"]
        DashboardState["状态服务\nDashboardState"]
        LocalYolo["本地检测服务\nLocalYoloService"]
        Config["配置服务\n检测阈值 / 演示模式"]
    ...
    subgraph Database["数据模块"]
        DevicesTable["devices"]
        EventsTable["events"]
        StatsTable["traffic_stats"]
        PlateTable["plate_records"]
        ModelTable["model_configs"]
    end
    ...
    Tracker --> Plate
    Plate --> Traffic
    Traffic --> DashboardState
    DashboardState --> StorageService
    StorageService --> DevicesTable
    StorageService --> EventsTable
    StorageService --> StatsTable
    StorageService --> PlateTable
    StorageService --> ModelTable
```

### 12.4 数据库 ER 图

第一版采用轻量 SQLite 或 JSONL 方案即可满足验收和复查需求。核心实体包括边缘设备、事件记录、交通统计、通行记录、白名单车辆和模型配置。后续若需要多用户或长期存储，可平滑迁移到 MySQL/PostgreSQL。

```mermaid
erDiagram
    DEVICES ||--o{ TRAFFIC_STATS : produces
    DEVICES ||--o{ EVENTS : reports
    DEVICES ||--o{ PLATE_RECORDS : captures
    WHITELIST ||--o{ PLATE_RECORDS : matches
    MODEL_CONFIGS ||--o{ TRAFFIC_STATS : generates
    MODEL_CONFIGS ||--o{ EVENTS : detects
