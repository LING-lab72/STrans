...
\pandocbounded{\includegraphics[keepaspectratio,alt={真实沙盘视频数据集总览}]{assets/contact-sheet.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={真实视频在前端实时监控页面中的运行画面}]{assets/frontend_realtime_live12.png}}
...
NFR-08 & 可测试性 & 纯逻辑使用确定性单测；数据库使用隔离数据；外部服务使用替身；真实视频只读并保留结构化输出 & 主项目 78 项、外部建模工具 22 项均通过 \\
...
\bottomrule\noalign{}
\endlastfoot
主项目自动化 & 78/78 & 已列服务、纯逻辑、前端和桥接行为无回归 & 所有 API 和需求均已覆盖 \\
道路建模原工具 & Python 17/17、Node 5/5 & 工具核心数据和脚本契约可重复 & 生产发布治理已自动化 \\
...
R-12 & 道路建模工具和 JSON 契约 & 桥接 6 项、原工具 22 项、页面无控制台错误 & 工具满足；发布仍需人工审核 \\
\end{longtable}
...
\item
  道路模型导出后仍需人工审核和版本留存；
\item
  100 个已执行测试/脚本单元全部通过，不表示需求覆盖率或算法准确率为 100\%。
\end{enumerate}
