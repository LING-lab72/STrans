...
\bottomrule\noalign{}
\endlastfoot
后端 Python & 56/56 通过 & 认证、存储、白名单、摄像头、调度和道路逻辑等已有用例 \\
前端 JavaScript & 16/16 通过 & 前端已有组件与逻辑用例 \\
项目内道路建模桥接 & 6/6 通过 & 桥接输入校验与本机约束 \\
道路建模原工具 & Python 17/17、Node 5/5 通过 & 原工具核心逻辑与脚本组 \\
浏览器系统遍历 & 17/17 检查点通过 & 双角色、主要页面和道路建模页面 \\
真实视频 CUDA & 13 段视频、65 帧全部成功读取和推理 & 数据链路、样例输出和当前环境性能 \\
\end{longtable}

上述合计 100 个自动化测试/脚本单元全部通过，但不表示需求、权限组合或算法准确率达到 100\%。当前真实视频没有完整人工框、车牌和事件真值，因而不得用检测输出数量代替 Precision、Recall、F1 或整牌准确率。
