...
cd D:\codeproject\STrans_remote_test\backend
...
\begin{Verbatim}[breaklines=true,breakanywhere=true,fontsize=\small,frame=single,framesep=2mm,rulecolor=\color{STransLine}]
cd D:\codeproject\STrans_remote_test\frontend
pnpm install
...
登录和注册需要图片验证码。验证码有效期为 5 分钟；前端在请求失败时按 1 秒、2 秒和 4 秒退避重试。管理员账号和初始密码由部署配置提供，建议通过环境变量设置强密码并在首次登录后核验。
...
\pandocbounded{\includegraphics[keepaspectratio,alt={前端实时监控操作界面}]{assets/frontend_realtime_live12.png}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={道路逻辑建模工具界面}]{assets/road-modeler-page.png}}
...
\begin{Verbatim}[breaklines=true,breakanywhere=true,fontsize=\small,frame=single,framesep=2mm,rulecolor=\color{STransLine}]
cd D:\codeproject\STrans_remote_test\backend
C:\Python314\python.exe -m pytest tests -q -W error::ResourceWarning

cd D:\codeproject\STrans_remote_test\frontend
pnpm test
pnpm build

cd D:\codeproject\STrans_remote_test
C:\Python314\python.exe -m pytest tools\road_logic_modeler\tests -q
\end{Verbatim}
